...
export function registerArtifactManagerCommands(
  context: vscode.ExtensionContext /* many parameters */
) {
...
export class FabricCommandManager implements IFabricCommandManager {
  ...
  private async createAndRegisterCommands(): Promise<void> {
    ...
    // const createCommand = new CreateArtifactCommand(this);
...
```typescript
export abstract class FabricCommand<T extends CoreTelemetryEventNames>
  implements IFabricCommand
{
...
- **Follow Pattern**: Use the command structure in README.md as template
